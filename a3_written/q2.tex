   ...
   In \texttt{parser\_model.py} you will find skeleton code to implement this simple neural network using PyTorch. Complete the \texttt{\_\_init\_\_}, \texttt{embedding\_lookup} and \texttt{forward} functions to implement the model. Then complete the \texttt{train\_for\_epoch} and \texttt{train} functions within the \texttt{run.py} file.
    ...
    \textbf{Deliverables:}
        \begin{itemize}
            ...
            \begin{answer}
                Best dev UAS: 88.58 and test UAS: 88.66
            \end{answer}
    \end{itemize}
...
\begin{answer}\newline
Disambiguation: Part-of-speech tags disambiguate words that have multiple possible meanings. For example, the word "bank" can be a noun meaning a financial institution, or a verb meaning to tilt to one side. However, if we know that "bank" is tagged as a noun, we can more confidently assume that it refers to a financial institution in a given context.
\\
Parsing efficiency: POS tags reduce parsing complexity and increase parsing efficiency. By identifying the part of speech of each word in a sentence, the parser can more easily identify the grammatical structure of the sentence and the relationships between its components.
\\
Improved accuracy: They provide additional information about the syntactic and semantic context of each word, which allows the parser to make more informed decisions about how to parse the sentence and what relationships to assign to each component.
\end{answer}
\end{parts}
