...
$\frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_w} 
    ...
    If $w = 0$: \\
    $- v_c + \frac{\exp(\bm u_{w}^\top v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top v_c)} v_c = (P(O=o \mid C=c) -1) v_c$ \\
    If $w \neq 0$: \\
    $- 0 + \frac{\exp(\bm u_{w}^\top v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top v_c)} v_c = P(O=o \mid C=c) v_c$ \\
    Therefore \\
    $\frac{\partial \bm J(\bm v_c, o, \bm U)}{\partial \bm u_w} = (\hat{\bm y} - \bm y)^\top v_{c} $
\end{answer}
\end{shaded}
