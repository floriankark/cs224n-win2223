...
\end{lstlisting}
    ...
    \ifans{The \texttt{enc\_masks} tensor is used to mask out the padded elements in the attention computation, ensuring that the decoder does not attend to them. This is achieved by setting the values of $\b e_t$ corresponding to the padded elements to $-\infty$. The softmax operation on $\b e_t$ then ensures that the attention scores corresponding to the padded elements become $0$, effectively ignoring them in the attention calculation. \\ It is necessary to use the masks in this way because the padded tokens in the source sequence have no meaning and should not be attended to by the decoder.} \newline
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{Corpus BLEU: 18.218032825693612}
    ...
    \ifans{
    \begin{enumerate}[i.]
        \item The advantage of dot product attention is that it is computationally more efficient, as it requires only a simple dot product operation between the query and the key vectors.\\ A disadvantage of dot product attention is that it does not allow the model to learn different weights for the query and key vectors, which limits its ability to capture complex patterns in the data.
        \item The advantage of additive attention is that it allows the model to learn different weights for the query and key vectors. This improves its ability to capture complex patterns in the data. Additionally, additive attention is more flexible than multiplicative attention, as it models non-linear relationships between the query and key vectors.\\ The disadvantage of additive attention is that it is computationally more expensive, as it requires multiple matrix multiplications and non-linear operations to compute the attention scores.
    \end{enumerate}
    }
\end{parts}
